...
def follow_field_source(model, path):
    """
    a model navigation chain "foreignkey.foreignkey.value" can either end with an actual model field
    ...
    if len(path) == 1:
        if isinstance(field_or_property, property):
            ...
            if DJANGO_VERSION.startswith('2'):
                ...
                # return getattr(model, field_or_property.field_name)
                # TODO
                return None
            else:
...
class AutoSchema(ViewInspector):
    ...
    def _map_serializer_field(self, method, field):
        ...
        if isinstance(field, serializers.ReadOnlyField):
            ...
            navigation_source = follow_field_source(field.parent.Meta.model, field.source_attrs)

            if callable(navigation_source):
                return self._map_type_hint(navigation_source)
            elif isinstance(navigation_source, models.Field):
                return self._map_model_field(navigation_source)
            else:
